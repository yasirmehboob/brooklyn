...
var dataString ='';
		...
		({
		type: "POST",
		url: "admin/ajax/php/inv_no.php?fnm=store_out_m",
		data: dataString,
		cache: false,
		dataType:'json',
...
$.post($("#form_sal").attr("action"),
	 ...
	 function(info){
		...
		if(info=="Saved.Saved."){//if successfully inserted into sales_out then insert auto voucher in accounts
			...
			debit:debit,
			crdit:crdit,
			descp:'System Auto Sales Voucher',
			chq_no:'n/a',
			chq_dt:'n/a',
			pyee:'n/a'}
			,function(av_data){
				if(av_data.status=='Saved.'){//if voucher successfully inserted then link the voucher number to the invoice in sale_out_m
					$.post('admin/ajax/php/up.php',{table:'store_out_m',field:'vnumb=\''+av_data.vnumb+'\'',condition:'mast_id='+$mast_id},
					function(iu_data){
						if(iu_data=="updated"){//if vnumb successfully linked to the sales then update balance stock in inventory prod tbl
